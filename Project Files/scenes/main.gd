...
func _on_score_left_body_entered(_body): #CPU Scores
	score[1] += 1
	$HUD/CPUScore.text = str(score[1])
	$GameEnd.play()
	if(score[1]>= 3):#Once a score reaches 3 the game ends
		$HUD/WinPanel/WinText.text = str("CPU WINS!")
		win_condition()
	else:
		$BallTimer.start(3)

func _on_score_right_body_entered(_body): #Player Scores
	score[0] += 1
	$HUD/PlayerScore.text = str(score[0])
	$GameEnd.play()
	if(score[0]>= 3):
		$HUD/WinPanel/WinText.text = str("PLAYER WINS!")
		win_condition()
	else:
		$BallTimer.start(3)
